...
class FeedScheduler:
    """Adaptive feed scheduler with priority-based polling and async support."""
    ...
    def _load_feed_configs(self) -> None:
    ...
    def start(self) -> None:
        ...
        self.logger.info("Initializing feed scheduler with async processing")
    ...
    def _poll_single_feed(self, feed_name: str, feed_url: str) -> None:
        """Poll an individual feed with async HTTP via asyncio.run."""
        asyncio.run(self._poll_single_feed_async(feed_name, feed_url))

    async def _poll_single_feed_async(self, feed_name: str, feed_url: str) -> None:
        """Async implementation of single feed polling."""
        start_time = time.time()
        try:
            self.logger.debug("Polling feed: %s", feed_name)

            feed_data = await self.fetcher.fetch_feed(feed_url, feed_name)
    ...
    def _process_successful_poll(self, feed_name: str, feed_url: str, feed_data: Dict) -> None:
    ...
    def _process_failed_poll(self, feed_name: str, feed_url: str) -> None:
        ...
        self._update_feed_interval(feed_name, int(backoff))
        FeedManager().update_feed_status(feed_name, {
    ...
    def _adjust_interval_based_on_activity(self, feed_name: str, feed_data: Dict) -> None:
    ...
    def _calculate_server_interval(self, feed_data: Dict) -> Optional[int]:
        ...
        if hasattr(feed_data, 'feed') and 'updated_parsed' in feed_data.feed:
            ...
            elif time_since_update < 86400:
                return 1800
        return None

    def _update_feed_interval(self, feed_name: str, new_interval: int) -> None:
